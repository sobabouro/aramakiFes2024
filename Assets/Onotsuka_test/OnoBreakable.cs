...
public class OnoBreakable : MonoBehaviour {
    // 固有のステータス
    ...
    private OnoSlash _slash;   // 切断処理クラス
    ...
    /// <returns></returns>
    public bool ReciveAttack(OnoBreaker breaker, int receivedATK, Type type) {
        // インターバル中ならスキップ
        if (_inDamageInterval)
            return false;
    ...
    /// </summary>
    /// <param name="breaker">`攻撃した側の情報</param>
    private void Break(OnoBreaker breaker, Type type) {
